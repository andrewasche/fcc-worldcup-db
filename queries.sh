...
echo "$($PSQL "SELECT t.name from games g join teams t on t.team_id = g.winner_id where year=2018 and round='Final';")"
...
echo "$($PSQL "SELECT g.year, t.name from games g join teams t on t.team_id = g.winner_id where round='Final' order by year;")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT distinct( t.name ) from teams t join games g on t.team_id = g.winner_id or t.team_id = g.opponent_id where t.name like 'Co%' order by t.name ")"
